...
import { runsSelector } from "./runs"

const reflectSecond = (_, second) => second

const projectsDictSelector = (state) => {
  return (
    state.projects.projects.data?.reduce((agg, project) => {
      agg[project.id] = project
      return agg
    }, {}) || {}
  )
}

// returns runs with their project names
export const runsWithNameSelector = createSelector(
  [runsSelector, projectsDictSelector],
  (runsState, projectsDict) => {
    const { data: runs } = runsState
    const runsWithName = runs.map((run) => ({
      ...run,
      projectName: projectsDict[run.projectId]?.name,
    }))
    return runsWithName
  },
)

export const runWithNameSelector = createSelector(
  [runsSelector, projectsDictSelector, reflectSecond],
  (runsState, projectsDict, runId) => {
    const { data: runs } = runsState
...
//       const paymentsForCreator = payments.filter((pmt) => pmt.recipient === crtr.id)
//       agg[crtr.id] = {
//         ...crtr,
...
//       const paymentsForGroup = payments.filter((pmt) => pmt.payment_group_id === pGroup.id)
//       agg[pGroup.id] = {
//         ...pGroup,
...
//       const completedPayments = payments.filter((pmt) => pmt.status === objectStates.succeeded)
...
//       const amountPosted = completedPayments.reduce((agg, pmt) => agg + pmt.amount, 0)
...
//     const todos = delinquentPayments.concat(delinquentContracts).concat(delinquentInvoices)
...
//     const recipientsWithPayments = Object.values(paymentRecipientsDict).reduce((agg, crtr) => {
//       const paymentsForRecipient = payments.filter((pmt) => pmt.recipient === crtr.id)
//       agg[crtr.id] = {
//         ...crtr,
...
//   [paymentsForCampaignSelector, deliverablesForCampaignSelector, creatorsDictSelector],
